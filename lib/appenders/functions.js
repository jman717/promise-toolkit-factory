...
module.exports = class functions extends base {
	...
	init() {
		...
		try {
			...
			for (i = 0; i < t.conf.map.length; i++) {
				...
				t.parent.log({ "type": "info", "text": `checking function(${t.conf.map[i]}) flow(${t.conf.flow})`, "classO": "functions.init", "file": "functions.js" });
	...
	get_function(jo) {
		...
		try {
			...
			t.parent.log({ "type": "error", "text": "error: " + e.message, "classO": "functions.get_function", "file": "functions.js" });
			throw e;
		}
	}
}