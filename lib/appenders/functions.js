...
module.exports = class functions extends base {
	...
	init() {
		...
		try {
			...
			var fa, ob, y, z, i, cl, fa0, fa1
			for (i = 0; i < t.conf.map.length; i++) {
				if (typeof t.conf.map[i] === 'undefined')
					throw new Error('t.conf.map[' + i + '] is not defined');
				t.parent.log({ "type": "info", "text": `checking function init(${t.conf.map[i]}) flow(${t.conf.flow})`, "classO": "functions.init", "file": "functions.js" })
				fa = t.conf.map[i].split('.');
				fa0 = fa[0];
				fa1 = fa[1];

				cl = t.parent.find_class(t.conf.map[i])
				if (cl === null)
	...
	get_function(jo) {
		...
		try {
			...
			if (typeof jo.search_for_function === 'undefined')
				throw new Error('jo.search_for_function is not defined');
			for (i = 0; i < t.conf.map.length; i++) {
				if (typeof t.conf.map[i] === 'undefined')
					throw new Error('t.conf.map[' + i + '] is not defined');
				if (t.conf.map[i] === jo.search_for_function) {
					return t.funcA[i];
				}
			}
			throw new Error('cannot find function(' + JSON.stringify(jo) + ')');
		} catch (e) {
			t.parent.log({ "type": "info", "text": "error: " + e.message, "classO": "functions.get_function", "file": "functions.js" });
			throw e
		}
	}
}