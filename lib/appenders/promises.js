...
module.exports = class promises extends base {
	...
	process(jo) {
		...
		try {
			...
			t.parent.log({ "type": "debug", "text": "process flow2=" + jo.flow, "classO": "promises.process", "file": "promises.js" });
	...
	promise_queue(jo) {
		...
		try {
			...
			try {
				if (jo.flow !== 'undefined')
					t.parent.log({ "type": "debug", "text": `flow(${jo.flow}) t.count(${t.count}) function(${fu}) function array(${JSON.stringify(t.appender.conf.map)})`, "classO": "promises.promise_queue", "file": "promises.js" });
				func = t.appender.getFlowAppender(jo).get_function({ "search_for_function": fu })    
			} catch (e) {
				t.parent.log({ "type": "debug", "text": "finished count(" + t.count + ") function(" + func + ")", "classO": "promises.promise_queue", "file": "promises.js" });
				//t.count = 0
				return;
			}
			new Promise((resolve, reject) => {
				...
				t.parent.log({ "type": "debug", "text": "process flow3=" + jo.flow, "classO": "promises.process", "file": "promises.js" });
				t.promise_queue(jo);
			}, reject => {
